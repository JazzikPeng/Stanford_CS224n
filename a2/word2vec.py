...
def negSamplingLossAndGradient(
    ...
    v_c, u_o, U = centerWordVec, outsideWordIdx, outsideVectors # U (V, N)
    v_c = v_c.reshape(-1, 1)  # Transform v_c to a column vector (N, 1)

    W = np.zeros([len(indices), U.shape[1]]) # (1  + 10 , N)
    for i in range(len(indices)):
        W[i] = U[indices[i]]
    pred = sigmoid(np.dot(W, v_c)) # (11, 1)
    y = np.zeros([len(indices), 1]) # (11, 1)
    y[0] = 1
    loss = (- y * np.log(pred) -  (1 - y)*np.log(1-pred)).sum()
    gradCenterVec = np.dot(W.T, pred-y).reshape(centerWordVec.shape)
    gradOutsideVecs = np.zeros_like(U) # (V, 3)
    gradW = np.dot(pred - y, v_c.T) # (K+1, N)
    for i in range(len(indices)):
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for word in outsideWords:
        ...
        loss_, gradCenterVec_, gradOutsideVecs_ = word2vecLossAndGradient(v_c, outsideWordIdx, U, dataset)
        gradCenterVecs[c_i] += gradCenterVec_
